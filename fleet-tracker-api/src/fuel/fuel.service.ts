...
import { CreateFuelEntryDto, FuelEntryType, FuelKpisQueryDto, FuelListQueryDto } from './dto/fuel.dto';
...
export class FuelService {
  ...
  async createEntry(userId: string, createFuelDto: CreateFuelEntryDto) {
    ...
    const trip = await this.tripRepository.findOne({ where: { id: createFuelDto.tripId, driverId: driver.id } });
    ...
    throw new BadRequestException({ code: 'INVALID_TYPE', message: 'Invalid fuel entry type' });
  }
  ...
  async getKpis(query: FuelKpisQueryDto) {
    ...
    const avgLitresPer100km = totalKm > 0 ? Number(((totalLitres / totalKm) * 100).toFixed(2)) : 0;
    const adblueConsumptionRate = totalKm > 0 ? Number(((adblueTotal / totalKm) * 100).toFixed(2)) : 0;
