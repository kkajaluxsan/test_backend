...
export class TripsService {
  constructor(
    ...
    @InjectRepository(LoadPickup)
    private readonly loadPickupRepository: Repository<LoadPickup>,
  ...
  async startTrip(userId: string, startTripDto: StartTripDto) {
    ...
    const truck = await this.truckRepository.findOne({
      where: { registrationNumber: startTripDto.truckNumber },
    });
    if (!truck)
      throw new NotFoundException({
  ...
  async addStops(userId: string, tripId: string, addStopsDto: AddStopsDto) {
    ...
    const stops = addStopsDto.stops.map((stop) =>
  ...
  async confirmLoad(
    ...
    this.fleetGateway.broadcastTripUpdated({
      tripId,
      status: trip.status,
      currentStop: stopId,
  ...
  async completeDelivery(
    ...
    tripId: string,
    stopId: string,
    completeDeliveryDto: CompleteDeliveryDto,
  ) {
    const trip = await this.getDriverTrip(userId, tripId);
  ...
  async endTrip(userId: string, tripId: string, endTripDto: EndTripDto) {
    ...
    if (endTripDto.endingKm < trip.startingKm) {
      throw new BadRequestException({
        code: 'INVALID_KM',
        message: 'Ending KM cannot be less than Starting KM',
      });
    }
  ...
  async findOne(id: string) {
    ...
    const loadPickups = await this.loadPickupRepository.find({
      where: { tripId: id },
    });
    const deliveryRecords = await this.deliveryRecordRepository.find({
      where: { tripId: id },
    });
  ...
  private async getTripStop(
    tripId: string,
    stopId: string,
    expectedType: StopType,
  ) {
    ...
    if (stop.type !== expectedType) {
      throw new BadRequestException({
        ...
        message: `Expected ${expectedType} stop`,
      });
    }
    return stop;
  }
  ...
  private async getTripSummary(tripId: string) {
    ...
    const deliveries = await this.deliveryRecordRepository.count({
      where: { tripId },
    });
